...
get_permafrost_parameters <- function(retune=FALSE){
  ...
  # calc CMIP6 fracs and show tuning results to that as well
  # read in CMIP6 fracs from csv
  ...
  # non_pfc <- param_default[[NONPF_C()]]
  # veg_val <- non_pfc*get_lm_prediction(model_list[["veg"]], non_pfc)
  # soil_val <- non_pfc*get_lm_prediction(model_list[["soil"]], non_pfc)
  # litter_val <- non_pfc*get_lm_prediction(model_list[["litter"]], non_pfc)
  # tot_c <- veg_val+soil_val+litter_val
  # extra_c <- non_pfc-tot_c
  # param_default[["pf_soil"]] <- max(soil_val + (soil_val/tot_c)*extra_c,0)  # using tot_c not value so that proportions add to 1
  # param_default[["pf_veg"]] <- max(veg_val + (veg_val/tot_c)*extra_c,0)
  # param_default[["pf_det"]] <- max(litter_val + (litter_val/tot_c)*extra_c,0)
